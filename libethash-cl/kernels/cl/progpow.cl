...
#define HASHES_PER_GROUP (WORKSIZE / PROGPOW_LANES)
...
// Causes OpenCL Error: clEnqueueNDRangeKernel: CL_INVALID_WORK_GROUP_SIZE (-54)
// on Ellesmere / OpenCL 1.2
...
)
{
    ...
    for (uint32_t word = lid*2; word < PROGPOW_CACHE_WORDS; word += WORKSIZE*2)
    {
    ...
    if (keccak_f800(g_header, seed, result) <= target) {
		...
		g_output->rslt[slot].mix[0] = result.uint32s[0];
		g_output->rslt[slot].mix[1] = result.uint32s[1];
		g_output->rslt[slot].mix[2] = result.uint32s[2];
		g_output->rslt[slot].mix[3] = result.uint32s[3];
		g_output->rslt[slot].mix[4] = result.uint32s[4];
		g_output->rslt[slot].mix[5] = result.uint32s[5];
		g_output->rslt[slot].mix[6] = result.uint32s[6];
		g_output->rslt[slot].mix[7] = result.uint32s[7];
	}
}
...
__kernel void GenerateDAG(uint start, __global hash64_t const* g_light, __global hash64_t * g_dag, uint light_size, uint isolate)
{
